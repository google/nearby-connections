// Copyright 2020 Google LLC
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     https://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

// WARNING: Please don't edit this file. It was generated by C++/WinRT
// v2.0.210505.3

#ifndef WINRT_Windows_Media_Capture_Core_1_H
#define WINRT_Windows_Media_Capture_Core_1_H
#include "winrt/impl/Windows.Media.Capture.Core.0.h"
WINRT_EXPORT namespace winrt::Windows::Media::Capture::Core {
  struct __declspec(empty_bases) IVariablePhotoCapturedEventArgs
      : winrt::Windows::Foundation::IInspectable,
        impl::consume_t<IVariablePhotoCapturedEventArgs> {
    IVariablePhotoCapturedEventArgs(std::nullptr_t = nullptr) noexcept {}
    IVariablePhotoCapturedEventArgs(void* ptr,
                                    take_ownership_from_abi_t) noexcept
        : winrt::Windows::Foundation::IInspectable(ptr,
                                                   take_ownership_from_abi) {}
    IVariablePhotoCapturedEventArgs(
        IVariablePhotoCapturedEventArgs const&) noexcept = default;
    IVariablePhotoCapturedEventArgs(
        IVariablePhotoCapturedEventArgs&&) noexcept = default;
    IVariablePhotoCapturedEventArgs& operator=(
        IVariablePhotoCapturedEventArgs const&) & noexcept = default;
    IVariablePhotoCapturedEventArgs& operator=(
        IVariablePhotoCapturedEventArgs&&) & noexcept = default;
  };
  struct __declspec(empty_bases) IVariablePhotoSequenceCapture
      : winrt::Windows::Foundation::IInspectable,
        impl::consume_t<IVariablePhotoSequenceCapture> {
    IVariablePhotoSequenceCapture(std::nullptr_t = nullptr) noexcept {}
    IVariablePhotoSequenceCapture(void* ptr, take_ownership_from_abi_t) noexcept
        : winrt::Windows::Foundation::IInspectable(ptr,
                                                   take_ownership_from_abi) {}
    IVariablePhotoSequenceCapture(
        IVariablePhotoSequenceCapture const&) noexcept = default;
    IVariablePhotoSequenceCapture(IVariablePhotoSequenceCapture&&) noexcept =
        default;
    IVariablePhotoSequenceCapture& operator=(
        IVariablePhotoSequenceCapture const&) & noexcept = default;
    IVariablePhotoSequenceCapture& operator=(
        IVariablePhotoSequenceCapture&&) & noexcept = default;
  };
  struct __declspec(empty_bases) IVariablePhotoSequenceCapture2
      : winrt::Windows::Foundation::IInspectable,
        impl::consume_t<IVariablePhotoSequenceCapture2> {
    IVariablePhotoSequenceCapture2(std::nullptr_t = nullptr) noexcept {}
    IVariablePhotoSequenceCapture2(void* ptr,
                                   take_ownership_from_abi_t) noexcept
        : winrt::Windows::Foundation::IInspectable(ptr,
                                                   take_ownership_from_abi) {}
    IVariablePhotoSequenceCapture2(
        IVariablePhotoSequenceCapture2 const&) noexcept = default;
    IVariablePhotoSequenceCapture2(IVariablePhotoSequenceCapture2&&) noexcept =
        default;
    IVariablePhotoSequenceCapture2& operator=(
        IVariablePhotoSequenceCapture2 const&) & noexcept = default;
    IVariablePhotoSequenceCapture2& operator=(
        IVariablePhotoSequenceCapture2&&) & noexcept = default;
  };
}
#endif
